...
struct _screenParamsDirectDraw {
	unsigned int iWidth = 0;
	unsigned int iHeight = 0;
	unsigned int iBitsPerPixel = 0;
	unsigned int iGraphicQuality = 0;
...
public:
	...
	int Initialize(_screenParamsDirectDraw* params, HWND hWnd);
...
private:
	LPDIRECTDRAW7 _lpdd7 = nullptr;
	LPDIRECTDRAWSURFACE7 _lpddsurf = nullptr;//main surface
	LPDIRECTDRAWSURFACE7 _lpddbacksurf = nullptr;//back buffer surface
	LPDIRECTDRAWCLIPPER _lpddclip = nullptr;
	...
	HDC _HDC = nullptr;
	HWND _hWnd = nullptr;
	void* _videoBuffer = nullptr;
	int _lPitch = 0;
	bool isBufferLocked = false;
	_screenParamsDirectDraw _sp = {};

	FlipDDStrategy* _flipStrategy = nullptr;
	DrawStrategy* _draw = nullptr;
	RenderState state = {};

	int InitializeDrawStrategies(_screenParamsDirectDraw* params);
	int ClipLine(int& x1, int& y1, int& x2, int& y2, _screenParamsDirectDraw* params);
